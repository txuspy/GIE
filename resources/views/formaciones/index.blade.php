					...
					<tr>
						<td>
							...
							@if(Session::get('search')=='1')
								target="_blank"
							@endif
							>{{ $formacion->$titulo }}</a><br> 
							<i><small>{{ $formacion->usuario?$formacion->usuario->name:'' }} {{ $formacion->usuario?$formacion->usuario->lname:'' }} 
								{{ \App\Traits\Listados::fechasIndex($formacion)}}
							</small></i>
						</td>
						<td>
							{{ $formacion->lugar }}
