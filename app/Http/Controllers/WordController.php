...
class WordController extends Controller
{
	...
	public function create(Request $request)
	{
		...
		*/
		...
		$this->crearTitulo($section, $phpWord, $request, __('IRAKASKUNTZA-JARDUERA'));
		...
		if (in_array("4", $request['secciones'])) {
			$this->wordProgramaDeIntercambio( $section, $request, 'internacional', $phpWord);
		...
		$this->crearTitulo($section, $phpWord, $request, mb_strtoupper(__('AZPko Formazioa')));
		...
		$this->crearTitulo($section, $phpWord, $request, __('IKERKUNTZA-JARDUERA'));
	...
	public function indiceWord($phpWord, $request )
	{
	   ...
	   // $myTextElement->setFontStyle($fontStyle);
		...
		if (in_array("13", $secciones)) {
			$section->addListItem( __('Unibertsitate aurreko ikasleei bideratutako ekintzak'), 0, null, 'multilevel');
		}
		if (in_array("14", $secciones)) {
			$section->addListItem( __('Eskolako ikasleei bideratutako ekintzak'), 0, null, 'multilevel');
			$section->addListItem( __('Bidelaguntza'), 1, null, 'multilevel');
			$section->addListItem( __('Formakuntza Osagarriak'), 1, null, 'multilevel');
		}
		if (in_array("4", $secciones)) {
			$section->addListItem( __('Elkartrukeko programak'), 0, null, 'multilevel');
			$section->addListItem( __('Elkartrukeko programak / mugikortasuna'), 1, null, 'multilevel');
			// $section->addListItem( __('IIP / AZPren mugikortasuna'), 1, null, 'multilevel');
		}
...
*/
		...
		if (in_array("3", $secciones)) {
			...
			$section->addText( mb_strtoupper( __('AZPko Formazioa') ), array('name' => $this->fuente,'size' => 13, 'bold' => true) );
			$section->addListItem( __('Jasotako formakuntza'), 0, null, 'multilevel');
			$section->addListItem( __('Emandako formazioa'), 0, null, 'multilevel');
		}
		...
		if (in_array("12", $secciones)) {
			...
			$section->addListItem( __('Artikuloak'), 1, null, 'multilevel');
		}

		if (in_array("4", $secciones)) {
			$section->addListItem( __('Elkartrukeko programak'), 0, null, 'multilevel');
			$section->addListItem( __('Egonaldiak / Beste Unibertsitateetan bisita'), 1, null, 'multilevel');
		}
		...
		if (in_array("15", $secciones)) {
			$section->addListItem( __('Gizarte-erantzukizuneko ekintzak'), 0, null, 'multilevel');
		}
		
		if (in_array("16", $secciones)) {
			$section->addListItem( __('Eskolaren hedakuntza'), 0, null, 'multilevel');
			$section->addListItem( __('Eskolaren hedakuntza'), 1, null, 'multilevel');
			$section->addListItem( __('Prentsa'), 1, null, 'multilevel');
		}
	...
	public function wordDivulgacion( $section, $request, $tipo, $phpWord)
	{
		...
		// if(count($divulgaciones)){
		if( !$divulgaciones->isEmpty() ){
			if( $tipo == 'hedakuntza' ){
				$tituloH1 = __('Eskolaren hedakuntza') ;
			}else{
				$tituloH1 = __('Prentsa');
			}
			$section->addText( __('Hedakuntza') , $this->styleH1  );
			...
			foreach ($divulgaciones as $divulgacion){
				$section->addText(  $divulgacion->$titulo , $this->styleH2  );
				// $section->addText(  $divulgacion->$desc , $this->styleP  );
	...
	//	dd($ekintzak);
		...
		if(!$ekintzak->isEmpty()){
			if( $tipo == 'laguntza' ){
				$tituloH1 = __('Bidelaguntza') ;
			}else{
				$tituloH1 = __('Formakuntza Osagarriak');
			}
			...
			foreach ($ekintzak as $ekintza){
				$section->addText(  $ekintza->$titulo."fff" , $this->styleH2  );
	...
	public function wordEkintzakAurretik( $section, $request, $phpWord)
	{
		...
		if(!$ekintzakAurretik->isEmpty()){
			$section->addText( __('Unibertsitate aurreko ikasleei bideratutako ekintzak') , $this->styleH1  );
			$lang      = $request->lng ;
			$titulo    = "titulo_".$lang ;
			$desc      = "desc_".$lang ;
			
			foreach ($ekintzakAurretik as $ekintzaAurretik){
				$section->addText(  $ekintzaAurretik->$titulo , $this->styleH2  );
	...
	public function wordEkintzakGizartea( $section, $request, $phpWord)
	{
		...
		if( !$ekintzakGizartea->isEmpty() ){
			...
			foreach ($ekintzakGizartea as $ekintzaGizartea){
				$section->addText(  $ekintzaGizartea->$titulo , $this->styleH2  );
	...
	public function Formaciones( $section, $request, $tipo, $modo, $phpWord)
	{
		...
		if( !$formaciones->isEmpty() ){
			...
			$section->addText( $tituloH1, $this->styleH1  );
			$section->addText( $tituloH2 , $this->styleH2  );
	...
	public function wordProgramaDeIntercambio( $section, $request, $tipo, $phpWord)
	{
		...
		if( !$programasDeIntercambios->isEmpty() ){
			...
			$section->addText( __('Elkartrukeko programak'), $this->styleH1  );
			$section->addText( $tituloH1, $this->styleH2  );
...
//die( $lineaInv );
	...
	public function wordProyectos( $section, $request, $tipo, $phpWord)
	{
		...
		if( !$proyectos->isEmpty() ){
			...
			$lang = \Session::get('locale');
			$lang = $request->lng ;
			$section->addText( __('Elkartrukeko programak / mugikortasuna') , $this->styleH1  );
			$section->addText( $tituloH1 , $this->styleH2  );
			$tableName     = $tituloH1;
			$phpWord->addTableStyle($tableName, $this->tableStyle);
			$table           = $section->addTable($tableName);
	...
	public function wordPublicacion( $section, $request, $tipo,  $phpWord)
	{
		...
		if( !$publicaciones->isEmpty() ){
			...
			$section->addText( __('Argitalpenak') , $this->styleH1  );
			$section->addText( $tituloH1 , $this->styleH2  );
			$phpWord->addTableStyle($tableName, $this->tableStyle);
			$table     = $section->addTable($tableName);
			// Tabla contenido variable
	...
	public function wordEquipoNuevo( $section, $request, $phpWord)
	{
		...
		if( !$equiposNuevos->isEmpty() ){
			...
			$lang = \Session::get('locale');
			$lang = $request->lng ;
			$section->addText( __('Hornikuntza Zientifikoa eskuratzea') , $this->styleH1  );
			// Tabla config
	...
	private function setStyles(){
		...
		$this->styleH2         = array('name' => $this->fuente, 'size' => 13, 'bold' => true);
